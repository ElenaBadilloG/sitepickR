...
We can also take a look at a table with the actual SMD for each covariate of interest applying the summary() function:
```{r}
unitBalanceTab = summary(m.out, balMeasure=1)
unitBalanceTab 
```
...
```{r}
matchBalanceTab = summary(m.out, balMeasure=2)
matchBalanceTab
```
...
Similary, use summary() function to obtain balance directly in a table:

```{r}
matchCountTab = summary(m.out, balMeasure=3)
matchCountTab
```
...
```{r}
subUnitBalanceTab = summary(m.out, balMeasure=4)
subUnitBalanceTab
```