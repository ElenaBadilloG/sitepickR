...
1.  __Study sample design__, where we:
    - Identify a target population with a nested structure (e.g. [district : school])
    - Set key parameter values (e.g. initial sample size, number of matches)
...
3. Obtain a list of __best K matches__ for each initially selected unit, where match #1 is closest to a given original unit, in terms of the covariates of interest and key paramneter values. These K matches will be the potential replacement units for each initially selected unit, in case their corresponding original unit is unable to participate in the study.
...
At this point, the two main outputs of the selectMatch procedure have been produced for our, and are stored inside the matchOut. In our example, these are: 
