...
1. A table with 10 matches (replacement candidates) for each of the 100 schools districts we initially selected. The matches are ordered from the most to the least 'similar' to their original district (where 'similar' is close, in terms of covariate distance).
...
We can also take a look at a table with the actual SMD for each covariate of interest applying the  [getSummary()](https://sitepickr.github.io/sitepickR-website/reference/getSummary.html) function:
